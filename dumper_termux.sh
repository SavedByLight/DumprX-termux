...
# ============================================================================
# Termux compatibility shim
# ============================================================================
# This toolkit (DumprX) upstream only targets glibc Debian/Ubuntu and Alpine.
# The pieces below patch the parts that don't otherwise work under Termux
# (aarch64 Android/Bionic, no root, no sudo, no `detox`/`rename` packages).
#
# IMPORTANT - things that remain broken even after this shim:
# The prebuilt CLI binaries cloned from ShivamKumarJha/android_tools
# (simg2img, lpunpack, packsparseimg, unsin, afptool, rkImageMaker,
# nb0-extract, aml-upgrade-package-extract, transfer, fsck.erofs,
# f2fs-extractor, RUU_Decrypt_Tool) are x86_64 glibc Linux ELF binaries.
# They will NOT execute under Termux. This shim auto-fetches a real
# Android/Bionic arm64-v8a `magiskboot` from Magisk's own releases (Magisk
# ships one since it manipulates boot images on-device), and falls back
# from `7zz` to Termux's own `p7zip` build. Everything else in that list
# has no verified aarch64-Android build available upstream; those code
# paths will print an error and skip rather than crash. If you find/build
# aarch64 Android replacements, drop them in "${UTILSDIR}/bin/" under the
# same filenames and this script will prefer them automatically.
...
# `detox`/`inline-detox` aren't packaged for Termux; provide minimal stand-ins
# that only get defined if the real tools aren't already on $PATH.
if ! command -v detox > /dev/null 2>&1; then
	_detox_one() {
		local f="$1" base dir new
		[[ -e "$f" ]] || return 0
		base=$(basename -- "$f"); dir=$(dirname -- "$f")
		new=$(printf '%s' "$base" | tr ' ()[]{}!' '________')
		[[ "$new" != "$base" ]] && mv -n -- "$f" "$dir/$new"
	}
	detox() {
		local recursive=0 args=() a
		for a in "$@"; do [[ "$a" == "-r" ]] && recursive=1 || args+=("$a"); done
		for t in "${args[@]}"; do
			[[ -e "$t" ]] || continue
			if [[ $recursive -eq 1 && -d "$t" ]]; then
				find "$t" -depth -print0 | while IFS= read -r -d '' f; do _detox_one "$f"; done
			else
				_detox_one "$t"
			fi
		done
	}
	inline-detox() { tr ' ()[]{}!' '________'; }
fi

# Best-effort fetch of a real Android/Bionic arm64 magiskboot, since the one
# cloned via android_tools is x86_64 Linux and won't run here.
_termux_fetch_magiskboot() {
	local dest="${UTILSDIR}/bin/magiskboot"
	...
	rm -f "$tmpzip"
	[[ -x "$dest" ]] || echo "WARNING: Could not fetch magiskboot; boot/recovery unpacking will fail."
}
...
if command -v 7zz > /dev/null 2>&1; then
	BIN_7ZZ=7zz
elif [[ "${IS_TERMUX}" -eq 1 ]] && command -v 7z > /dev/null 2>&1; then
	# android_tools' bundled 7zz is x86_64 Linux and won't run under Termux;
	# p7zip's `7z` (installed via `pkg install p7zip`) accepts the same
	# switches this script uses (l -ba, e -y, x -y, -snld) so it's a safe
	# drop-in.
	BIN_7ZZ=7z
else
...
[[ "${IS_TERMUX}" -eq 1 ]] && _termux_fetch_magiskboot
...
if [[ $(${BIN_7ZZ} l -ba "${FILEPATH}" | grep -i aml) ]]; then
	...
	# `rename` (Perl-style) isn't packaged for Termux; equivalent bash renames:
	for f in *.PARTITION; do [[ -e "$f" ]] && mv -- "$f" "${f%.PARTITION}.img"; done
	for f in *_aml_dtb.img; do [[ -e "$f" ]] && mv -- "$f" "${f%_aml_dtb.img}dtb.img"; done
	for f in *_a.img; do [[ -e "$f" ]] && mv -- "$f" "${f/_a.img/.img}"; done
	if [[ -f super.img ]]; then
...
if ${BIN_7ZZ} l -ba "${FILEPATH}" | grep -q "system.new.dat" 2>/dev/null || [[ $(find "${TMPDIR}" -type f -name "system.new.dat*" -print | wc -l) -ge 1 ]]; then
	...
	for partition in $PARTITIONS; do
		...
		${BIN_7ZZ} e -y "${FILEPATH}" ${partition}.*.new.dat* ${partition}.*.transfer.list ${partition}.*.img 2>/dev/null >> ${TMPDIR}/zip.log
		# `rename` (Perl-style) isn't packaged for Termux; bash-regex equivalent below.
		...
		for f in *; do
			[[ "$f" =~ ^([A-Za-z0-9_]+)\.([0-9]+)\.(.*)$ ]] && mv -n -- "$f" "${BASH_REMATCH[1]}.${BASH_REMATCH[3]}"
		done
...
for p in $PARTITIONS; do
	...
	# Fall back to mount loop (skipped on Termux: no `sudo`, and loop-mounting
	# a filesystem needs root + kernel loop/erofs/f2fs driver access that an
	# unrooted Termux install simply doesn't have)
	if [[ "${IS_TERMUX}" -eq 1 ]]; then
		echo "ERROR: Could not extract '$p' partition (7z/fsck.erofs/f2fs-extractor all failed)."
		echo "Loop-mount fallback is unavailable on Termux (no sudo/root loop mount)."
		continue
	fi
