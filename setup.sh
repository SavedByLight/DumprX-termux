...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    if command -v apt > /dev/null 2>&1; then
	    ...
	    sleep 1
        sudo apt install -y unace unrar zip unzip p7zip-full sharutils rar uudeview mpack arj cabextract device-tree-compiler liblzma-dev python3-pip brotli lz4 axel gawk aria2 detox cpio rename liblz4-dev jq git-lfs || abort "Setup Failed!"
